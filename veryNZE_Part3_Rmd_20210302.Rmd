...
This document reports on the analysis of the adjective amplifier very in New Zealand English. The analysis was performed with the aim of fitting a model (generalized-linear binomial logistic mixed-effects regression) to explain which factors correlate with the use of very to ascertain if very is decreasing across apparent time and if this retreat is systematic or layered (which would imply that the use of very is stratified along linguistic and social lines) or unsystematic (which would imply that it correlates with few if any linguistic or social variables). 

If you use this script or results thereof, please cite it as:
Anonymous, Anonymous. 2019. On the waning of forms - a corpus-based analysis of decline and loss in the New Zealand English amplifier system. 
...
We proceed by loading the data, factorizing cariables that are so far represented as character strings instead of factors, and inspecting its characteristics.
